...
tf.app.flags.DEFINE_float(  'gpu_memory_fraction', 0.7, 'GPU memory fraction to use.')
# scaffold related configuration
tf.app.flags.DEFINE_string('data_dir', config_args.train_tfrecord_dir,'The directory where the dataset input data is stored.')
tf.app.flags.DEFINE_integer('num_classes', config_args.num_classes, 'Number of classes to use in the dataset.')
tf.app.flags.DEFINE_string('model_dir', config_args.logdir, 'The directory where the model will be stored.')
...
tf.app.flags.DEFINE_integer( 'save_checkpoints_secs', 600, 'The frequency with which the model is saved, in seconds.')
...
tf.app.flags.DEFINE_integer( 'max_number_of_steps', 1000000,  'The max number of steps to use for training.')
tf.app.flags.DEFINE_integer( 'batch_size', config_args.batch_size,  'Batch size for training and evaluation.')
...
tf.app.flags.DEFINE_string('decay_boundaries', '3000, 5000, 10000, 20000, 40000',
    'Learning rate decay boundaries by global_step (comma-separated list).')
tf.app.flags.DEFINE_string('lr_decay_factors', '1, 0.5, 0.2, 0.1, 0.05, 0.02',
    'The values of learning_rate decay factor for each segment between boundaries (comma-separated list).')
# checkpoint related configuration
tf.app.flags.DEFINE_string('checkpoint_path', config_args.pretrained_path,  'The path to a checkpoint from which to fine-tune.')
tf.app.flags.DEFINE_string('checkpoint_model_scope', config_args.checkpoint_model_scope,  'Model scope in the checkpoint. None if the same as the trained model.')
tf.app.flags.DEFINE_string('model_scope', 'ssd300',  'Model scope name used to replace the name_scope in checkpoint.')
tf.app.flags.DEFINE_string('checkpoint_exclude_scopes', 'ssd300/multibox_head, ssd300/additional_layers, ssd300/conv4_3_scale',
...
def get_init_fn():
    ...
    return scaffolds.get_init_fn_for_scaffold(FLAGS.model_dir, FLAGS.checkpoint_path,
                                            ...
                                            name_remap={'/kernel': '/weights', '/bias': '/biases', '/weights': '/'})
...
def input_pipeline(dataset_pattern='train-all*', is_training=True, batch_size=FLAGS.batch_size):
    def input_fn():
        ...
        anchor_area = np.load('./anchor_area.npy')
        ...
        image_preprocessing_fn = lambda image_, labels_, bboxes_ : ssd_preprocessing.preprocess_image(image_, labels_, bboxes_, config_args.bias, out_shape, is_training=is_training, data_format=FLAGS.data_format, output_rgb=True)
...
def ssd_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=tf.AUTO_REUSE):
        ...
        backbone = ssd_net.MobileNetV2(params['data_format'])
    ...
    for trainable_var in tf.trainable_variables():
        if 'BatchNorm' not in trainable_var.name:
            if 'expanded_conv_5_scale' not in trainable_var.name:
    ...
    # total_loss = tf.add(cross_entropy, tf.multiply(params['weight_decay'], tf.add_n(l2_loss_vars), name='l2_loss'), name='total_loss')
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        optimizer = tf.train.AdamOptimizer(learning_rate=truncated_learning_rate)
...
def gen_anchor_area_mask():
    ...
    anchor_encoder_decoder = anchor_manipulator.AnchorEncoder(allowed_borders = [1.0] * config_args.layer_num,
                                                    ...
                                                    ignore_threshold = FLAGS.neg_threshold,
                                                    prior_scaling=[0.1, 0.1, 0.1, 0.1] )
    anchor_area = anchor_encoder_decoder._get_area(all_anchors, all_num_anchors_depth, all_num_anchors_spatial)

    sess = tf.Session()
    init_op = tf.group([tf.local_variables_initializer(), tf.local_variables_initializer(), tf.tables_initializer()])
    sess.run(init_op)
    area = sess.run(anchor_area)
    np.save('./anchor_area.npy', area)

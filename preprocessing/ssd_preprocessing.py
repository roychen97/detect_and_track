...
def ssd_random_expand(image, bboxes, bias, ratio=2., name=None):
  with tf.name_scope('ssd_random_expand'):
    ...
    mean_color_of_image = [bias[0]*2./255., bias[1]*2./255., bias[2]*2./255.]#tf.reduce_mean(tf.reshape(image, [-1, 3]), 0)
...
def ssd_random_sample_patch_wrapper(image, labels, bboxes, bias):
  with tf.name_scope('ssd_random_sample_patch_wrapper'):
    orgi_image, orgi_labels, orgi_bboxes = image, labels, bboxes
    def check_bboxes(bboxes):
      areas = (bboxes[:, 3] - bboxes[:, 1]) * (bboxes[:, 2] - bboxes[:, 0])
    ...
    def body(index, image, labels, bboxes, orgi_image, orgi_labels, orgi_bboxes):
      image, bboxes = tf.cond(tf.random_uniform([], minval=0., maxval=1., dtype=tf.float32) < 0.5,
                      ...
                      lambda: ssd_random_expand(orgi_image, orgi_bboxes, bias, tf.random_uniform([1], minval=1.1, maxval=4., dtype=tf.float32)[0]))
      # Distort image and bounding boxes.
...
# def random_resize(images,labels, bboxes):
...
def unwhiten_image(image, bias):
  means=bias
  num_channels = image.get_shape().as_list()[-1]
  channels = tf.split(axis=2, num_or_size_splits=num_channels, value=image*255)
  for i in range(num_channels):
    channels[i] += means[i]
  return tf.concat(axis=2, values=channels)
...
def random_resize(image):
  image = tf.cond(tf.random_uniform([], minval=0., maxval=1., dtype=tf.float32) < 0.5,
                      lambda: image,
                      lambda: tf.image.resize_images(image, size=[64,64], method=0))
  angle = tf.random_uniform([], minval=-20., maxval=20., dtype=tf.float32)
  tf.contrib.image.rotate(image, angle * math.pi / 180, interpolation='BILINEAR')
  tf.contrib.image.rotate(image, - angle * math.pi / 180, interpolation='NEAREST')
  return image


def random_rotate(image, bboxes):
  angle = tf.random_uniform([], minval = -math.pi, maxval = math.pi, dtype=tf.float32)
  ...
  rotation_matrix = tf.stack([-tf.sin(angle),  tf.cos(angle),
                      tf.cos(angle), tf.sin(angle)], axis=0)                      
  rotation_matrix = tf.reshape(rotation_matrix, (2,2))
  size_hw =  tf.cast([tf.shape(image)[0:2]], tf.float32)
  # center = tf.concat([bboxes[:,1] / size_hw[1], bboxes[:,0]/size_hw[0]], axis=1)
  center = bboxes[:,0:2]
  center = tf.matmul(center - 0.5, rotation_matrix) + 0.5
  bboxes = tf.concat([center[:,::-1], bboxes[:,2:]], axis=1)
  return image, bboxes


def preprocess_for_train(image, labels, bboxes, out_shape, bias, data_format='channels_first', scope='ssd_preprocessing_train', output_rgb=True):
  ...
  with tf.name_scope(scope, 'ssd_preprocessing_train', [image, labels, bboxes]):
    ...
    # random_sample_image, labels, bboxes = ssd_random_sample_patch_wrapper(distort_image, labels, bboxes, bias)
    ...
    # random_sample_flip_resized_image = tf.image.resize_images(random_resize(random_sample_flip_image), out_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR, align_corners=False)
    # random_sample_flip_resized_image.set_shape([None, None, 3])
    rotated_image, bboxes = random_rotate(random_sample_flip_image, bboxes)
    random_sample_flip_resized_image = tf.image.resize_images(random_resize(rotated_image), out_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR, align_corners=False)
    final_image = tf.to_float(tf.image.convert_image_dtype(random_sample_flip_resized_image, orig_dtype, saturate=True))
    final_image = _mean_image_subtraction(final_image, bias)
    final_image = final_image * 2.0 / 255.0
...
def preprocess_for_eval(image, out_shape, bias, data_format='channels_first', scope='ssd_preprocessing_eval', output_rgb=True):
  ...
  with tf.name_scope(scope, 'ssd_preprocessing_eval', [image]):
    ...
    image = _mean_image_subtraction(image, bias)
...
def preprocess_image(image, labels, bboxes, bias, out_shape, is_training=False, data_format='channels_first', output_rgb=True):
  ...
  """
  if is_training:
    return preprocess_for_train(image, labels, bboxes, out_shape, bias, data_format=data_format, output_rgb=output_rgb)
  else:
    return preprocess_for_eval(image, out_shape, bias, data_format=data_format, output_rgb=output_rgb)
